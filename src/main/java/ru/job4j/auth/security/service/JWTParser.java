...
import ru.job4j.auth.error.JWTParseException;
...
public class JWTParser {
    ...
    public DecodedJWT parseToken(String token) {
        if (token == null || !token.startsWith(TOKEN_PREFIX)) {
            throw new JWTParseException("JWT-token must starts with Bearer prefix");
        }
        DecodedJWT decodedJWT = verifier.verify(token.replaceAll(TOKEN_PREFIX, ""));
        if (getRoles(decodedJWT).isEmpty()) {
            throw new JWTParseException("JWT-token does not contain any role");
        }
        if (getUsername(decodedJWT) == null) {
            throw new JWTParseException("JWT-token does not contain claim id");
        }
        return decodedJWT;
    }
    ...
    public String getUsername(DecodedJWT decodedJWT) {
        Integer rsl = decodedJWT.getClaim("id").asInt();
